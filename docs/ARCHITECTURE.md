...
- Case-study images render via `components/ui/Figure.tsx` as a plain `<img>` (not `next/image` — see that component's own comment); every image referenced from frontmatter carries required `alt` text (schema-enforced) and an `assetType: "real-screenshot" | "verified-diagram" | "provisional-illustration"` field (D-019, `docs/DECISIONS.md`; CONTENT_MODEL §2/§6). Zero CLS is instead guaranteed by the asset's own `width`/`height` attributes matching its aspect ratio — required for every asset type, not just raster ones (CONTENT_MODEL §4).
- Case-study images live in `public/images/projects/<slug>/`.
- **No invented or AI-generated final screenshots, ever — this rule is unconditional and D-019 does not relax it.** Until Hakan provides approved real screenshots, a project may instead publish with repository-verified `verified-diagram`/`provisional-illustration` SVGs (D-019) — a visibly labelled, honestly-captioned, repository-traced diagram or illustration, never a fake UI, invented screen, or generated-to-look-like-a-screenshot image. A generic locally created flat placeholder (`PLACEHOLDER — asset pending`) remains the fallback for content that hasn't yet reached either real screenshots or D-019 diagrams; it's allowed in drafts and blocked in published content by the truthfulness gate unless the surrounding copy honestly labels it.
- Every registered image is validated by the reusable D-019 asset gate (`checkImageAssets`, `lib/content/validate.ts`, CONTENT_MODEL §6 item 6) — path safety (project-relative, no traversal), an extension allowed for its `assetType`, on-disk existence, and (for `verified-diagram`/`provisional-illustration`) an honestly-worded caption. This gate checks labelling and structure, never factual correctness — that a diagram accurately depicts what it claims to remains a repository-audit and human-review responsibility, exactly like every other publication gate in this project.
