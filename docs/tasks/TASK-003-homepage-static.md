...
Status: **COMPLETE (2026-08-10, homepage-image completion pass)** — full gate suite green (Chromium + WebKit); **9 of 9 acceptance criteria now genuinely met**. **History**: implemented 2026-07-31 with 8 of 9 criteria met, 1 honestly left unchecked (no project images rendered anywhere on the homepage). Updated 2026-08-10 (JointLedger publication pass) to complete the D-016 order, with the image criterion still deliberately unchecked (unrelated to project publication). This pass closes that final gap: `components/project/ProjectCard.tsx` (shared by `/` Selected Systems and `/work`) now renders each project's first registered `images[]` entry as a truthful, D-019-compliant representative asset — see the reconciled criterion below for full evidence. Awaiting review. **Execution order revised 2026-07-17: this task now runs after TASK-004 and depends on it (D-011 rejected).**
...
- [x] Selected systems order matches approved D-016: Kıvılcım, DropSpot, JointLedger, Professional Systems preview; the professional entry uses the exact D-009 pending copy; Kıvılcım is displayed per D-017 (first English introduction may gloss “Spark”). **Met (2026-08-10)**: JointLedger published this pass (`order: 3`, `depth: "short"`, `verificationStatus: "verified"`), completing the D-016 order with all four `featured`-tier entries now `status: "published"` and rendering in the correct sequence — Kıvılcım (`order: 1`), DropSpot (`order: 2`), JointLedger (`order: 3`), Professional Systems (`order: 4`, exact D-009 pending copy, unchanged).
- [x] Every image is a labelled placeholder SVG — no fake UI, no stock, no generated screenshots. **Superseded requirement (D-019, 2026-08-05)**: this criterion assumed the only acceptable pre-real-asset image would be the generic "PLACEHOLDER — ASSET PENDING" SVG. Hakan's D-019 decision explicitly replaces that assumption: a project may instead use repository-verified, visibly labelled `verified-diagram`/`provisional-illustration` assets (a strictly more informative honesty-preserving alternative to the generic pending panel), alongside real screenshots where they exist. **Current approved criterion (2026-08-05)**: every rendered image is either a real screenshot (`assetType: "real-screenshot"`), or a visibly labelled `verified-diagram`/`provisional-illustration` (D-019) whose content traces only to repository-verified facts — no fake UI, no stock imagery, no generated screenshots, in either case. **Still not exercised on the homepage itself (as recorded 2026-08-10, JointLedger publication pass)**: the homepage's "Selected systems" section (`components/sections/`) renders zero project images/thumbnails — confirmed by inspection, no `Figure`/`images[]` usage anywhere in `app/` or `components/sections/`. TASK-005 (this decision's companion task) does now render real D-019 assets, but only inside the Kıvılcım case-study page itself, not on the homepage. Left unchecked at that point for the same reason as before — nothing on the homepage exercised this criterion either way — not weakened or reworded beyond recording the supersession.

  **Current approved criterion, homepage-specific (2026-08-10, homepage-image completion pass)**: each project shown in Selected Systems includes one truthful, accessible, D-019-compliant representative project asset. **Met, independently verified for all four homepage project entries (2026-08-10)**: `components/project/ProjectCard.tsx` — shared, unchanged between `/` Selected Systems and `/work`, so there is exactly one image-selection code path — now renders each project's first registered `images[]` entry via the existing `Figure` component (same soft-paper mat, corner ticks, and honest mono caption used everywhere else on the site), capped to a secondary, non-dominant width. No project slug is hard-coded into the selection logic; the image simply comes from whichever entry is already first in that project's own `images[]` array. Concretely: Kıvılcım → `product-areas-map.svg` (`provisional-illustration`); DropSpot → `browse-drops.webp` (`real-screenshot`); JointLedger → `upstream-extension-map.svg` (`verified-diagram`); Professional Systems → a newly authored `professional-systems-overview.svg` (`provisional-illustration`), built only from its own approved public description and carrying the required visible label "PROVISIONAL EDITORIAL VISUAL — NOT A PRODUCT SCREENSHOT". All four already existing project entries kept their pre-existing `images[]` order unchanged — each one's preferred homepage asset already happened to be its first registered image, so no reordering or file duplication was needed. All four pass the reusable `checkImageAssets`/`validatePublicationGates` gates (`lib/content/validate.ts`), re-run directly against live frontmatter. No new content-model field was introduced — "first registered image" is used exactly as PROJECT_SPEC's existing `images[]` array already models it, since no separate cover-image concept previously existed.
