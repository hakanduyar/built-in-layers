...
- [ ] `/work` renders tiers correctly in D-016 order (Kıvılcım, DropSpot, JointLedger, Professional preview); drafts excluded; JointLedger preview shows upstream disclosure text. **Updated (2026-08-05, TASK-006), still unmet, honestly**: Kıvılcım, DropSpot, and Professional Systems are now published and render in the correct D-016 relative order on `/work`; JointLedger alone remains `status: "draft"` pending Hakan's confirmation of its public/private repository decision and safe wording (`docs/CONTENT_INVENTORY.md`) — see `docs/PROGRESS.md`. Left unchecked until JointLedger can honestly satisfy it too; not weakened or reworded.
- [ ] Published preview entries (kivilcim, dropspot, professional-systems) contain zero unverified claims — only approved seed/pending copy; professional pending copy is the exact D-009 wording. **Unmet as literally written, updated 2026-08-05**: this criterion's own premise — three *preview-depth* entries — no longer describes any current state of the site. Kıvılcım and DropSpot did move to draft after this criterion was written (as the note above recorded), but have since been published again via TASK-005/TASK-006 as fully-audited `depth: "full"`/`"short"` case studies, not as the shallow preview entries this criterion checked; Professional Systems alone remains an actual preview and still satisfies its half (zero claims, exact D-009 wording). Left unchecked rather than reinterpreted to fit the new state — the criterion as written doesn't have three preview entries to check against anymore, in either direction.
